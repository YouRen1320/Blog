<template>
  <!--
    /tags/[slug] —— 某标签下的已发布文章。
  -->
  <section class="page">
    <header class="head">
      <NuxtLink to="/writing" class="back mono">← 全部文章</NuxtLink>
      <div class="mono kicker">TAG · {{ data?.meta.total ?? 0 }} 篇</div>
    ...
    <div v-else-if="(data?.meta.total ?? 0) === 0" class="empty mono">这个标签下还没有文章。</div>
...
import { useArticlesByTag } from '../../composables/useArticles'
import { frenchSeason, monthDay } from '../../utils/format'

const route = useRoute()
const slug = route.params.slug as string

const { data, status, error } = await useArticlesByTag(slug, { pageSize: 100 })

useSeoMeta({
  title: `标签 · #${slug} · YouRen`,
  description: `「#${slug}」标签下的所有已发布文章。`,
})
