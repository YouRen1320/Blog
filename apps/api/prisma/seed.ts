 ...
 * 跑法:`pnpm seed:demo`(已经在根 .env 注入 DATABASE_URL 后调用)
 *
 * 设计原则:
 * - 幂等:重复跑不会报错(用 upsert)
...
async function main() {
  // This check intentionally runs before PrismaClient is constructed.
  assertDemoSeedAllowed(process.env);
  const demoAdmin = readDemoAdminConfig(process.env);
  ...
  try {
    ...
    const admin = await prisma.user.upsert({
      ...
      // Demo identities may be reset so Playwright and the seed stay in sync.
      update: {
...
如果你能在网页上看到这篇,说明从数据库到接口到前端的整条链路都跑通了。`,
    ...
    console.log('✅ Seed 完成');
    console.log(`   E2E 管理员:${demoAdmin.email}`);
