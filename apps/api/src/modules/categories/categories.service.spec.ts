...
describe('CategoriesService', () => {
  ...
  it('create:无 slug 时自动从 name 生成', async () => {
    prismaMock.category.create.mockImplementation(({ data }: any) => Promise.resolve({ id: 'c1', ...data }));
    await service.create({ name: 'Hello World' });
    expect(prismaMock.category.create.mock.calls[0][0].data.slug).toBe('hello-world');
  });

  it('create:显式给的 slug 优先', async () => {
    prismaMock.category.create.mockImplementation(({ data }: any) => Promise.resolve({ id: 'c1', ...data }));
    await service.create({ name: 'Anything', slug: 'custom' });
    expect(prismaMock.category.create.mock.calls[0][0].data.slug).toBe('custom');
  });
